...
class ViewProducts : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editTextSearch.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(charSequence: CharSequence?, start: Int, before: Int, count: Int) {
                // Aquí realizaremos la búsqueda de productos según el texto ingresado.
        ...
        // Configura el Adapter con la lista de productos vacía
        productAdapter = ProductAdapter(productList)
        productAdapter.setOriginalProductList(productList)

        recyclerView.adapter = productAdapter

        // Obtiene la lista de productos mediante Retrofit
        fetchProductList(1)
    }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == ADD_PRODUCT_REQUEST_CODE && resultCode == Activity.RESULT_OK) {
            // Obtener la lista de productos actualizada solo si se agregó un nuevo producto
            fetchProductList(1)
        }
    }

    fun onVerDetallesProducto1(productId: Int) {
        val intent = Intent(this, ViewDetailsProduct::class.java)
        intent.putExtra(ViewDetailsProduct.EXTRA_PRODUCT_ID, productId)
        startActivity(intent)
    }

    private fun fetchProductList(categoryId : Int) {
        ...
        call.enqueue(object : Callback<List<Product>> {
            override fun onResponse(call: Call<List<Product>>, response: Response<List<Product>>) {
                if (response.isSuccessful) {
                    ...
                    if (products != null) {
                        // Actualiza la lista de productos y notifica al Adapter
                        productList.clear()
                        productList.addAll(products)
                        productAdapter.notifyDataSetChanged()
                        println("Valor de products: $products")
                    }
                } else {
